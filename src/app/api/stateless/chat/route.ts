...
export async function POST(request: Request) {
  ...
  const limit = await rateLimit("ai", requestIdentifier(request));
  if (!limit.success) {
    return NextResponse.json({ error: "AI question limit reached. Please try again after reset." }, { status: 429 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Gemini interpretation is temporarily unavailable." },
      { status: 503 },
    );
  }
}
